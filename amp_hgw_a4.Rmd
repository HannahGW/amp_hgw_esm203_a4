...
In 2000, the Santa Barbara Coastal Term Ecological Long-Term Research (SBC LTER) program was established to better understand kelp forest ecosystems in southern California (SBC LTER). Spiny lobsters (*Panulirus interruptus*, Figure 1), which are a focus of one of many SBC LTER studies, are an important predator in gaint kelp forests (Reed, 2019). Monitoring lobster abundance, size, and fishing pressure are part of an annual, on-going study that started in 2012 until, most recently, 2018. Studies were conducted at five sites near Santa Barabra County: Carpinteria, Isla Vista, Arroyo Quemado, Mohawk, and Naples (Figure 2). On January 1, 2012, marine protected areas (MPAs) were established at Isla Vista and Naples. 
...
Data for spiny lobster abundance, size, and fishing efforts was collected by divers from the months of August to December from 2012 to 2018 by the [Santa Barbara Coastal Long-Term Ecological Research (LTER)](http://sbc.lternet.edu//index.html). Lobster counts and sizes were based on visual observations from a diver searching 2.5 m areas on opposite sides of a 60 m transect tape.  For more information on collection methods and spatial information, please see the [metadata](https://portal.edirepository.org/nis/mapbrowse?scope=knb-lter-sbc&identifier=77&revision=newest).

Trends in lobster counts and sizes were visually compared in MPA and non-MPA. Counts and sizes (mm) of lobsters were compared in Marine Protected Areas (MPA) and Non-Marine Protected Areas (non-MPA). The differences in mean lobster size in MPAs and non-MPAs were compared by two-sample t-tests ($\alpha$ = 0.05 throughout). All analyses and figures were prepared using R software version 3.6.1. Collaboration between authors was done using GitHub. 
...
***Figure 3.** Lobster counts in MPAs (red) and non-MPAs (black) from 2012 to 2018. MPA lobster counts in 2012 were close to zero, and increased to over 250 individuals at Naples MPA and over 800 individuals at Isla Vista MPA. Data: SBC LTER.*


#### Result B: Lobster size distribution shifts, 2012 vs 2018

The lobster size distribution in the five study sites in 2012 and 2018 is shown in Figure 4. Lobster size distribution has stayed relatively constant for two non-MPAs in 2012 and 2018: Carpinteria (n~2012~= 78, n~2018~=343) and Mohawk (n~2012~= 83, n~2018~=164). The lobster sizes followed a normal distribution and the means were 74.4mm (SE~2012~ +- 1.65mm) and 77.3mm (SE~2012~ +- 1.2mm), respectively. The other MPA, Arroyo Quemado (n~2012~=38, mean~2012~=71mm, SE~2012~=1.7mm), had a bi-modal distribition in 2012; the two modes occuring around 65mm and 90mm. However, in 2018 lobster sizes in Arroyo Quemado followed a similar normal distribtion like the other non-MPAs. 

In 2012, lobster sizes at both MPAs, Isla Vista (n~2012~=26) and Naples (n~2012~=6), were right skewed. Isla vista lobsters had an average size of 66.1 +- 2.4mm (mean +- SE) and lobsters at Naples had an average size of 73mm +- 4.8mm. However, in 2018 the distribution of lobster sizes (Isla Vista n~2018~=946 76.6mm +-0.4mm, Naples n~2018~=298 80.5mm +-0.5mm) followed a normal distribition that is similar to that of the other non-MPA sites in 2018. 

In 2018, all sites followed a normal distribition. Naples had the largest mean lobster size in 2018 and Isla Vista had the second largest mean lobster size. Small sample size at Naples and Isla Vista could account for the skewed distribution. Arroyo Quemado also had a small sample size compared to the other non-MPAs. Likely, the larger number of observations displayed the true population distribution for all the sites.
...
For observations in 2012, mean lobster sizes between MPA (`r round(mpa_nonmpa_ttest_2012$estimate[1],2)`mm) and non-MPA sites (`r round(mpa_nonmpa_ttest_2012$estimate[2],2)`mm) varied significantly (t(`r round(mpa_nonmpa_ttest_2012$parameter,2)`) = `r round(mpa_nonmpa_ttest_2012$statistic,2)`, *p* = `r round(mpa_nonmpa_ttest_2012$p.value,2)`) according to the p-value. To measure the maginitude of the difference between the means, we calculated the effect size (`r round(Q1_effectsize$estimate,1)`). The medium effect size revealed there was a significant difference between mean lobster sizes at MPA and non-MPA sites in 2012.
...
In MPA sites, the difference in mean lobster size observations in 2012 (`r round(MPA_size_ttest$estimate[1],2)`mm) and 2018 (`r round(MPA_size_ttest$estimate[2],2)`mm) varied significantly (t(`r round(MPA_size_ttest$parameter,2)`) = `r round(MPA_size_ttest$statistic,2)`, *p* = `r round(MPA_size_ttest$p.value,2)`). The large effect size (`r round(Q3_effectsize$estimate,1)`) for MPA sites revealed that the difference in lobster means in 2012 and 2018 was indeed substantial.  

In non-MPA sites, the difference in mean lobster size observations in 2012 (`r round(Non_MPA_size_ttest$estimate[1],2)`mm) and 2018 (`r round(Non_MPA_size_ttest$estimate[2],2)`mm) did not vary significantly (t(`r round(Non_MPA_size_ttest$parameter,2)`) = `r round(Non_MPA_size_ttest$statistic,2)`, *p* = `r round(Non_MPA_size_ttest$p.value,2)`). When looking at the acutal means of the two samples, we determined that the p-value indicated that the null hypothesis was true; 2012 and 2018 means did not differ significantly. 

### Summary

It is worth noting that all size observations recorded by divers were visually estimated. Actual lobster size varies and are subject to human error. Effect sizes recorded below do not account for the accuracy of assessment (Sullivan and Feinn, 2012). 
...
- Population means between 2012 and 2018 in non-Marine Protected Areas did not vary significantly (Table 1)
