...
The impact of creating Marine Protected Areas (MPAs) on lobster abundance can be seen in Figure 2. At the Isla Vista MPA, lobster abundance dramatically increased over 3,500% from 13 to 472 observed lobsters from 2012 to 2018. While lobster abundance at the Naples MPA wasn't as high as that at Isla Vista MPA in 2018 (n=149), Naples had a larger percent increase (over 4,800% increase) from 2012-2018. Comparatively, lobster abundance at the non-MPA sites, Carpinteria, Mohawk, and Arroyo Quemado, only had a 341%, 42%, and 100% increase from 2012-2018, respectively.  


The total number of lobsters in MPAs increased over 3,700% from 16 lobsters in 2012 to 621 lobster observations in 2018, while the bumber of observed lobsters only increased 183% from 99 lobsters in 2012 to 281 lobsters in 2018.  

However, these changes in lobster abundance weren't as apparent until about 2014. 
```{r}
# Install packages
...
# Question 3:	For MPA sites only, is there a significant difference in lobsters observed in 2012 vs. 2018?
...
For observations in 2012, mean lobster sizes between MPA (`r round(size_ttest_2012$estimate[1],2)`) and Non-MPA sites (`r round(size_ttest_2012$estimate[2],2)`) did not differ significantly (t(`r round(size_ttest_2012$parameter,2)`) = `r round(size_ttest_2012$statistic,2)`, *p* = `r round(size_ttest_2012$p.value,2)`). The 95% confidence interval for MPAs (`r round(size_ttest_2012$conf.int[1],2)`) vary from the confidence interval of Non-MPAs (`r round(size_ttest_2012$conf.int[2],2)`). Because of the discrepency between the confidnce intervals, the p-value is not the best indiciation of the significance of mean differences... FINISH EXPLANATION: list actual differences, % changes, and measure of uncertainty. 

For observations in 2018, mean lobster sizes between MPA (`r round(size_ttest_2018$estimate[1],2)`) and Non-MPA sites (`r round(size_ttest_2018$estimate[2],2)`) did vary significantly (t(`r round(size_ttest_2018$parameter,2)`) = `r round(size_ttest_2018$statistic,2)`, *p* = `r round(size_ttest_2018$p.value,2)`). The confidence intervals for MPAs (`r round(size_ttest_2018$conf.int[1],2)`) and Non-MPAs (`r round(size_ttest_2018$conf.int[2],2)`) are much smaller than the ones calculate above, which is a good indication that the p-value is representative of the mean differences between MPAs and Non-MPAs.
FINISH EXPLANATION...

In MPA sites, the difference in lobster observations in 2012 
